...
GSExact_basic=function(input_data, J,n, burn, iter){
  ...
  return(list(mean_mat, var_mat, ci_mat, mc.mat))
}


GSExact_long=function(input_data, J,n, burn, iter){
  ...
  init_lmer=lmer(vij~ x1 + x2 + x3 + x1:x2 + q2 + q3 + q4 + (1|id), data=final_data)
  lme=summary(init_lmer)
  ...
  B2=lme$coefficients[3,1]
  B3=lme$coefficients[4,1]
  B5=lme$coefficients[5,1]
  B6=lme$coefficients[6,1]
  B7=lme$coefficients[7,1]
  B4=lme$coefficients[8,1]

  Beta=matrix(c(B0,B1,B2,B3,B4,B5,B6,B7),5,1)

  xv2_mat = matrix(0, nrow=5, ncol=J)

  for(i in 1:J){
    xv2_mat[,i] = as.matrix(c(1,x1[i], x2[i], x3[i], x1[i]*x2[i]),5,1)
  }

  xv1_mat=matrix(cbind(rep(0,3),diag(3)), 3,4)
  ...
  save.beta0=save.beta1=save.beta2=save.beta3=save.beta4=save.beta5=save.beta6=save.beta7=NULL
  save.tau=save.sig=NULL
  save.Tmat=array(0, dim=c(2,2,SIM))
  save.Alpha=array(0, dim=c(4,1,SIM))
  ...
  for (k in 1:SIM){
    ...
    Bmat2 = matrix(c(B0,B1,B2,B3,B4),5,1)
    Bmat1 = matrix(c(B5,B6,B7),3,1)
    one = rep(1,J)
    Xmat = rbind(one, x1, x2, x3, x1*x2)
    ...
    for (j in 1:J){
      one=matrix(1,4,1)
      dj_mat=vobs_mat[[j]] - one%*%t(Xmat[,j])%*%Bmat2 - t(xv1_mat)%*%Bmat1
      meanj=varj*(1/Sig)*sum(dj_mat)
      uj_mat[j]= rnorm(1,meanj, sqrt(varj))
    }
    ...
    for (i in 1:J){
      Xj_mat[[i]] = matrix( cbind(matrix(1,4,1)%*%Xmat[,i], t(xv1_mat)), n, 8)
    }
    ...
    B0=Beta[1]; B1=Beta[2]; B2=Beta[3]; B3=Beta[4]; B4=Beta[5] ; B5=Beta[6]; B6=Beta[7]; B7=Beta[8];
    ...
    save.beta3=rbind(save.beta3, B3)
    save.beta4=rbind(save.beta4, B4)
    save.beta5=rbind(save.beta4, B5)
    save.beta6=rbind(save.beta4, B6)
    save.beta7=rbind(save.beta4, B7)
    ...
    final_data$XB_vec=rep(XB, each=4)
    final_data$eij=final_data$vij-final_data$XB_vec-final_data$uj_vec-B5*final_data$q2-B6*final_data$q3-B7*final_data$q4
    ...
    ### Not use Mvj but we are using updated x1 and x2
    e_ij=rnorm((n*J),0,sd=sqrt(Sig))
    vij=ifelse(vm==1, final_data$XB_vec+final_data$uj_vec+B5*final_data$q2+B6*final_data$q3+B7*final_data$q4+e_ij, final_data$vij)
    final_data$vij=vij
    ...
    for (j in 1:J){
      if(x1_missing[j]==0){imp_x1[j]=x1[j]}
        ...
        mean_j=meanx[j]+vmat[,,j][n+1,1:n]%*%solve(vmat[,,j][1:n,1:n])%*%(vobs_mat[[j]]-one4*meany[j] - t(xv1_mat)%*%Bmat1)
        var_j=vmat[,,j][n+1,n+1]-vmat[,,j][n+1,1:n]%*%solve(vmat[,,j][1:n,1:n])%*%vmat[,,j][1:n,1+n]
        imp_x1[j]=rnorm(1, mean=mean_j, sd=sqrt(var_j))
      }
    }
    ...
    meany=B0+B1*x1+B3*x3+uj+(B2+B4*x1)*Mu_x2x1  # common part for j
    meanx=Mu_x2x1
  ...
  mc.mat=matrix(rbind(mc.tau, mc.sig, mc.beta0, mc.beta1, mc.beta2, mc.beta3, mc.beta4), 7, iter-burn+1)
